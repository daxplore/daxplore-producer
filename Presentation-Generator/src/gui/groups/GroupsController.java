...
public class GroupsController implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		case GROUPS_ADD_ACTION_COMMAND:
			String groupName = (String) JOptionPane.showInputDialog(
					guiMain.getGuiMainFrame(), "Name:", "Create new group",
					JOptionPane.PLAIN_MESSAGE, null, null, "");
			if (groupName != null && !groupName.equals("")) {
				try {
					MetaGroupManager metaGroupManager = guiMain.getGuiFile()
							.getDaxploreFile().getMetaData()
							.getMetaGroupManager();
					int nextid = metaGroupManager.getHighestId() + 1;
					TextReferenceManager textReferenceManager = guiMain
							.getGuiFile().getDaxploreFile().getMetaData()
							.getTextsManager();
					TextReference tr = textReferenceManager.get("Group"
							+ nextid);
					tr.put(groupName, new Locale("sv")); // TODO: fix global
															// locale
					MetaGroup mg = metaGroupManager.create(tr,
							Integer.MAX_VALUE, GroupType.QUESTIONS,
							new LinkedList<MetaQuestion>());
					TreePath treepath = groupTreeModel.addGroup(mg,
							groupTreeModel.getChildCount(groupTreeModel
									.getRoot()));
					...
					JOptionPane.showMessageDialog(guiMain.getGuiMainFrame(),
							...
							"Group creation error", JOptionPane.ERROR_MESSAGE);
					e1.printStackTrace();
				}
			} else {
				JOptionPane.showMessageDialog(guiMain.getGuiMainFrame(),
						"Group has to have a name", "Group creation error",
		...
		case GROUPS_UP_ACTION_COMMAND:
			...
			try {
				if (path.length == 2) {
					int currentIndex = groupTreeModel.getIndexOfChild(path[0],
							path[1]);
					if (currentIndex > 0) {
						groupTreeModel.moveChild(path[1], path[0],
								currentIndex - 1);
						groupJTree.setSelectionPath(new TreePath(path));
					}
				} else if (path.length == 3) {
					int currentIndex = groupTreeModel.getIndexOfChild(path[1],
							path[2]);
					if (currentIndex > 0) {
						groupTreeModel.moveChild(path[2], path[1],
								currentIndex - 1);
						groupJTree.setSelectionPath(new TreePath(path));
					} else {
						int groupIndex = groupTreeModel.getIndexOfChild(
								path[0], path[1]);
						if (groupIndex > 0) {
							Object newGroup = groupTreeModel.getChild(path[0],
									groupIndex - 1);
							groupTreeModel.moveChild(path[2], newGroup,
									groupTreeModel.getChildCount(newGroup));
							groupJTree
									.setSelectionPath(new TreePath(
											new Object[] { path[0], newGroup,
													path[2] }));
						}
					}
				}
		...
		case GROUPS_REMOVE_ACTION_COMMAND:
			try {
				...
				groupTreeModel.removeChild(child); // the row that actually does
													// work, rest just what the
													// selection should be...
				int siblingCount = groupTreeModel.getChildCount(parent);
				List<Object> newPath = Arrays.asList(Arrays.copyOfRange(path,
						0, path.length - 2));
		...
		case ADD_TO_GROUP_ACTION_COMMAND:
			...
			for (QuestionWidget qw : questionJList.getSelectedValuesList()) {
				try {
					TreePath treepath = groupTreeModel.addQuestion(
							qw.metaQuestion, parent, atIndex);
					atIndex++;
					groupJTree.setSelectionPath(treepath);
				} catch (Exception e1) {
	...
	public void loadData() {
		if (guiMain.getGuiFile().isSet()) {
			try {
				MetaData md = guiMain.getGuiFile().getDaxploreFile()
						.getMetaData();
				questionListModel = new QuestionListModel(md);
				questionJList = new MouseOverList(questionListModel);
				// questionJList.setCellRenderer(groupsView.new
				// QuestionListCellRenderer());
				questionJList
						.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
				groupsView.getQuestionsScrollPane().setViewportView(
						questionJList);
				...
				groupJTree
						.setCellRenderer(groupsView.new GroupTreeCellRenderer());
				// tree.getPr
