...
public class ImportWizardController implements ActionListener {
	...
	public void actionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (evt.getActionCommand().equals(
				ImportWizardDialog.CANCEL_BUTTON_ACTION_COMMAND))
			cancelButtonPressed();
		else if (evt.getActionCommand().equals(
				ImportWizardDialog.BACK_BUTTON_ACTION_COMMAND))
			backButtonPressed();
		else if (evt.getActionCommand().equals(
				ImportWizardDialog.NEXT_BUTTON_ACTION_COMMAND))
			nextButtonPressed();
	}
	...
	private void cancelButtonPressed() {
		...
		// clear the spss file save just in case, when the dialog opens again we
		// don't
	...
	public void importSpssFileAction() {
		...
		// this should never happen the way the dialog is designed. But we keep
		// this for safety.
		if (hostPanel.getGuiMain().getGuiFile().getSpssFile() == null) {
			JOptionPane.showMessageDialog(this.hostPanel.getGuiMain()
					.getGuiMainFrame(),
		...
		try {
			...
			hostPanel.getGuiMain().getGuiFile().getDaxploreFile()
					.importSPSS(importFile, charset);

			// NOTE: Information on successful file import is updated elsewhere.

		} catch (FileNotFoundException e2) {
			JOptionPane.showMessageDialog(this.hostPanel.getGuiMain()
					.getGuiMainFrame(), "Unable to find the SPSS file.",
					"Daxplore file warning", JOptionPane.ERROR_MESSAGE);
			e2.printStackTrace();
			return;
		} catch (IOException e2) {
			JOptionPane.showMessageDialog(this.hostPanel.getGuiMain()
					.getGuiMainFrame(), "File import error.",
					"Daxplore file warning", JOptionPane.ERROR_MESSAGE);
			e2.printStackTrace();
			return;
