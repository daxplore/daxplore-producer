 ...
 */
public class ImportWizardDialog extends JDialog implements
	...
	public void setCurrentPanel(Object id) {
		...
		ImportWizardDescriptor oldPanelDescriptor = importWizardModel
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (evt.getPropertyName().equals(
				ImportWizardModel.CURRENT_PANEL_DESCRIPTOR_PROPERTY)) {
			importWizardController.resetButtonsToPanelRules();
		} else if (evt.getPropertyName().equals(
				ImportWizardModel.NEXT_FINISH_BUTTON_TEXT_PROPERTY)) {
			nextButton.setText(evt.getNewValue().toString());
		} else if (evt.getPropertyName().equals(
				ImportWizardModel.BACK_BUTTON_TEXT_PROPERTY)) {
			backButton.setText(evt.getNewValue().toString());
		} else if (evt.getPropertyName().equals(
				ImportWizardModel.CANCEL_BUTTON_TEXT_PROPERTY)) {
			cancelButton.setText(evt.getNewValue().toString());
		} else if (evt.getPropertyName().equals(
				ImportWizardModel.NEXT_FINISH_BUTTON_ENABLED_PROPERTY)) {
			nextButton.setEnabled(((Boolean) evt.getNewValue()).booleanValue());
		} else if (evt.getPropertyName().equals(
				ImportWizardModel.BACK_BUTTON_ENABLED_PROPERTY)) {
			backButton.setEnabled(((Boolean) evt.getNewValue()).booleanValue());
		} else if (evt.getPropertyName().equals(
				ImportWizardModel.CANCEL_BUTTON_ENABLED_PROPERTY)) {
	...
	public void close(int code) {
		...
		if (returnCode == FINISH_RETURN_CODE) {
			...
			getGuiMain().getOpenPanelView().getOpenController()
					.updateSpssFileInfoText();
			getGuiMain().getOpenPanelView().getOpenController()
					.updateTextFields();
		}
	...
	/**
	 * Code relevant to the creation of the dialog, basic layout, next, back and
	 * cancel buttons goes here. Descriptors are used as controllers and panel
	 * views for display in separate files.
	 */
	private void initcomponents() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(
				ImportWizardDialog.class
						.getResource("/gui/resources/Arrow-Up-48.png")));
		setDialog(this);
